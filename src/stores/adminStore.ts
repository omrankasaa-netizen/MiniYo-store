...
function currentUserName(): string {
  return getCurrentUser()?.name || 'Admin'
}
    ...
    (set) => ({
      ...
      updateProduct: (id, updates) => set(state => {
        ...
        return {
          products: updated,
          pendingChanges: true,
          auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'UPDATE', entity: 'Product', entityId: String(id), details: `Updated ${Object.keys(updates).join(', ')}`, user: currentUserName(), createdAt: new Date().toISOString() }]
        }
      }),
      ...
      deleteProduct: (id) => set(state => {
        ...
        return {
          ...
          pendingChanges: true,
          auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'DELETE', entity: 'Product', entityId: sid, details: 'Product deleted', user: currentUserName(), createdAt: new Date().toISOString() }]
        }
      }),
      ...
      bulkUpdateProducts: (ids, updates) => set(state => {
        ...
        return {
          ...
          pendingChanges: true,
          auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'BULK_UPDATE', entity: 'Product', entityId: ids.join(','), details: `Bulk ${Object.keys(updates).join(', ')}`, user: currentUserName(), createdAt: new Date().toISOString() }]
        }
      }),
      ...
      updateOrderStatus: (id, status) => set(state => {
        ...
        const newAuditLogs = [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'STATUS_CHANGE', entity: 'Order', entityId: id, details: `Status → ${status}`, user: currentUserName(), createdAt: new Date().toISOString() }]
        ...
        if (status === 'cancelled' && order && order.orderStatus !== 'cancelled') {
          ...
          for (const item of order.items) {
            ...
            if (idx >= 0) {
              ...
              newMovements.push({ id: `mov-${Date.now()}-${item.productId}`, productId: item.productId, productName: item.productName, type: 'return', quantity: item.quantity, previousStock: prevStock, newStock, note: `Stock restored from cancelled order ${order.orderNumber}`, createdBy: 'System', createdAt: new Date().toISOString() })
              newAuditLogs.push({ id: `audit-${Date.now()}-${item.productId}`, action: 'STOCK_RESTORE', entity: 'Inventory', entityId: String(item.productId), details: `Restored ${item.quantity} units from cancelled order ${order.orderNumber}`, user: 'System', createdAt: new Date().toISOString() })
            }
          }
      ...
      updateOrderPayment: (id, status) => set(state => ({
        ...
        pendingChanges: true,
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'PAYMENT_CHANGE', entity: 'Order', entityId: id, details: `Payment → ${status}`, user: currentUserName(), createdAt: new Date().toISOString() }]
      })),
      ...
      toggleWhatsAppConfirmed: (id) => set(state => ({
        ...
        pendingChanges: true,
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'WHATSAPP_TOGGLE', entity: 'Order', entityId: id, details: 'WhatsApp confirmation toggled', user: currentUserName(), createdAt: new Date().toISOString() }]
      })),
      ...
      deleteOrder: (id) => set(state => {
        ...
        return {
          ...
          pendingChanges: true,
          auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'DELETE', entity: 'Order', entityId: id, details: `Deleted order ${order?.orderNumber || id}`, user: currentUserName(), createdAt: new Date().toISOString() }],
        }
      }),
      ...
      restoreOrderStock: (orderId) => set(state => {
        ...
        for (const item of order.items) {
          ...
          if (idx >= 0) {
            ...
            const newStock = prevStock + item.quantity
            restoredProducts[idx] = { ...restoredProducts[idx], stockQuantity: newStock }
            newMovements.push({ id: `mov-${Date.now()}-${item.productId}`, productId: item.productId, productName: item.productName, type: 'return', quantity: item.quantity, previousStock: prevStock, newStock, note: `Manual stock restore from cancelled order ${order.orderNumber}`, createdBy: currentUserName(), createdAt: new Date().toISOString() })
            newAuditLogs.push({ id: `audit-${Date.now()}-${item.productId}`, action: 'STOCK_RESTORE', entity: 'Inventory', entityId: String(item.productId), details: `Manually restored ${item.quantity} units from cancelled order ${order.orderNumber}`, user: currentUserName(), createdAt: new Date().toISOString() })
          }
        }
      ...
      deleteCustomer: (id) => set(state => ({
        ...
        pendingChanges: true,
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'DELETE', entity: 'Customer', entityId: id, details: 'Customer deleted', user: currentUserName(), createdAt: new Date().toISOString() }],
      })),
      ...
      addMovement: (m) => set(state => {
        ...
        if (idx >= 0) newProducts[idx] = { ...newProducts[idx], stockQuantity: m.newStock }
        return {
      ...
      addMedia: (asset) => set(state => ({ media: [...state.media, asset], pendingChanges: true })),
      deleteMedia: (id) => set(state => ({ media: state.media.filter(m => m.id !== id), pendingChanges: true })),
      ...
      assignMediaToProduct: (mediaId, productId) => set(state => {
        ...
        const updatedProducts = state.products.map(p => {
          if (String(p.id) === pid || p.slug === pid) {
            return { ...p, images: [...(p.images || []), { id: `img-${Date.now()}`, url: mediaItem.url, alt: mediaItem.name, altAr: mediaItem.altAr || mediaItem.name, isPrimary: !(p.images || []).length }] }
          }
          return p
        })
        return {
          ...
          pendingChanges: true,
          auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'ASSIGN', entity: 'Media', entityId: mediaId, details: `Assigned to product ${pid}`, user: currentUserName(), createdAt: new Date().toISOString() }]
        }
      }),
      ...
      addAuditLog: (action, entity, entityId, details) => set(state => ({
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action, entity, entityId, details, user: currentUserName(), createdAt: new Date().toISOString() }]
      })),

      updateSettings: (s) => set(state => ({ settings: { ...state.settings, ...s }, pendingChanges: true })),

      setHeroImage: (url) => set(state => ({
        settings: { ...state.settings, heroImageUrl: url },
        pendingChanges: true,
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'SET_HERO', entity: 'Media', entityId: 'hero', details: 'Hero image updated', user: currentUserName(), createdAt: new Date().toISOString() }]
      })),

      setLogoImage: (url) => set(state => ({
        settings: { ...state.settings, logoImageUrl: url },
        pendingChanges: true,
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'SET_LOGO', entity: 'Media', entityId: 'logo', details: 'Logo image updated', user: currentUserName(), createdAt: new Date().toISOString() }]
      })),

      setFooterImage: (url) => set(state => ({
        settings: { ...state.settings, footerImageUrl: url },
        pendingChanges: true,
        auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'SET_FOOTER', entity: 'Media', entityId: 'footer', details: 'Footer image updated', user: currentUserName(), createdAt: new Date().toISOString() }]
      })),
      ...
      publishChanges: () => set(state => {
        ...
        return {
          publishedAt: now,
          pendingChanges: false,
          auditLogs: [...state.auditLogs, { id: `audit-${Date.now()}`, action: 'PUBLISH', entity: 'Store', entityId: 'all', details: `Published at ${now}`, user: currentUserName(), createdAt: now }]
        }
      }),
        ...
        if (result.ordersImported > 0 || result.stockProductsUpdated > 0) {
          try {
            ...
            if (raw) {
              ...
              set({ products: state.products || [], orders: state.orders || [], movements: state.movements || [], pendingChanges: true })
            }
          } catch { /* ignore */ }
        }
        return result
...
export const storefrontAPI = {
  getProducts(): Product[] { return useAdminStore.getState().products },
  getCategories() {
    const { products } = useAdminStore.getState()
    const catMap = new Map()
    products.forEach(p => { if (p.category && !catMap.has(p.category.id)) catMap.set(p.category.id, p.category) })
    return Array.from(catMap.values())
  },
  getProduct(handle: string): Product | null { return useAdminStore.getState().products.find(p => p.slug === handle) || null },
  getNewArrivals(): Product[] { return useAdminStore.getState().products.filter(p => p.isNew) },
  getBestSellers(): Product[] { return useAdminStore.getState().products.filter(p => p.isBestseller) },
  getFeatured(): Product[] { return useAdminStore.getState().products.filter(p => p.isFeatured) },
  getWishlistProducts(ids: string[]): Product[] { return useAdminStore.getState().products.filter(p => ids.includes(p.id)) },
  getCmsSection(key: string): CmsSection | undefined { return useAdminStore.getState().cmsSections.find(s => s.key === key && s.isActive) },
  getAllCmsSections(): CmsSection[] { return useAdminStore.getState().cmsSections.filter(s => s.isActive) },
  getSettings() { return useAdminStore.getState().settings },
