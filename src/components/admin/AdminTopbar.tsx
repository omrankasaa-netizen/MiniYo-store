...
import { Globe, Check, Bell, LogOut, ChevronDown } from 'lucide-react'
...
export function AdminTopbar({ title, onMenuClick }: Props) {
  ...
  // Notifications: new orders needing confirmation + any pending WA
    ...
    <>
      <header className="sticky top-0 z-30 bg-[#FAFAF7]/95 backdrop-blur-sm border-b border-[#E8E4DB] h-14 flex items-center px-4 lg:px-6 shrink-0">
        ...
        <div className="flex items-center gap-2">
          {/* Apply Changes Button — with confirm dialog */}
          {pendingChanges ? (
            <button
              ...
              <>
                ...
                <div className="absolute right-0 top-10 w-80 bg-white rounded-xl shadow-xl border border-[#E8E4DB] z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b border-[#E8E4DB] flex items-center justify-between">
                    ...
                    {notifCount > 0 && (
                      <span className="text-xs text-white bg-red-500 px-2 py-0.5 rounded-full">{notifCount} new</span>
                    )}
                  </div>
                        ...
                        {pendingOrders.map(o => (
                          <div key={o.id} className="px-4 py-3 hover:bg-[#F9F8F5]">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-xs font-semibold text-[#2D5A4C]">
                                  New order: {o.orderNumber}
                                </p>
                                <p className="text-xs text-[#8B8578]">
                                  {o.customerName} · ${o.grandTotal.toFixed(2)} · {o.paymentMethod.toUpperCase()}
                                </p>
                                <p className="text-[10px] text-[#A8A396] mt-0.5">
                                  {new Date(o.createdAt).toLocaleString('en-LB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        {pendingWA.map(o => (
                          <div key={o.id} className="px-4 py-3 hover:bg-[#F9F8F5]">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-xs font-semibold text-[#2D5A4C]">
                                  WhatsApp needed: {o.orderNumber}
                                </p>
                                <p className="text-xs text-[#8B8578]">
                                  {o.customerName} · Status: {o.orderStatus.replace(/_/g, ' ')}
                                </p>
                              </div>
                            </div>
                          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            ...
            <div className="flex gap-3 mt-2">
              <button
                ...
                className="flex-1 py-2 text-sm font-medium text-[#5C6B60] bg-[#F2EFE9] hover:bg-[#E8E4DB] rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmPublish}
                className="flex-1 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors"
              >
                Yes, Publish
              </button>
            </div>
